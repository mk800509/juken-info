...
1. ダッシュボードHTMLを更新してください。学校ごとに `<!-- SCHOOL:xxx --> ... <!-- /SCHOOL -->` で区切られたカードがあります。変化があった学校のカード内で新しい内容を反映するようテーブルの行やdivの文言を書き換え、変わった箇所には `updated`（divの場合 class="row updated"）または `updated-row`（trの場合 class="updated-row"）を追加してください。他の学校カードに残っている古いハイライトは通常のクラスに戻してください。該当する `<ul id="log-XXX">` の先頭に `<li>YYYY/MM/DD HH:MM 内容の要約</li>` を追加し最新5件までに保ってください。`<div id="last-checked">` も更新してください。
   - 各 `<table class="events">` 内の日付が確定している行（トは既存の他行と同様、複数日の行事は最終日を採用）には必ず `data-date="YYYY-MM-DD"` 属性を `<tr>` タグに付けてください（例: `<tr data-date="2026-09-19">`）。学校年度は2026年4月始まりなので、1〜3月の日付で「翌」表記がある場合は2027年、それ以外の月は2026年としてください。日付未確定の行（「未公開」「要確認」等で具体的な年月日が無い行）には付けないでください。新しく行を追加する場合も同様にこの属性を必ず付与してください。
   - HTML内の `<style>` ブロックにある `table.events tr.past-row` / `.past-tag` のCSSと、`</body>` 直前の過去イベント自動判定用 `<script>` ブロック、および凡例(`.legend`)内の「終了済みのイベントには...」の文言は、削除・変更せずそのまま残してください（この仕組みにより、日付を過ぎた行は自動的に薄く表示され「終了」タグが付きます）。
   - 各カードにある `<div class="memo">...</div>`（備忘録／家族用メモ）ブロックは、家族が手動で書いた個人的なメモです。中身は絶対に書き換えたり削除したりせず、既存のカード内の位置（`.note` の後、`.changelog` の前）にそのまま残してください。新しいメモが追加で必要になった場合を除き、あなたが自動でメモの文言を追加・変更することはありません。
